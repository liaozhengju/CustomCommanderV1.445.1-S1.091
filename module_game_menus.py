...
#          (assign, "$waiting_for_arena_fight_result", 0),
        ...
        (try_begin), # same mnu_town
          ...
          (call_script, "script_party_get_ideal_size", "$current_town"),
          (try_begin),
            (eq, ":center_lord", "trp_player"),           
            (str_store_string,s11,"@ Your own banner flies over the castle gate.Current garrison limit:{reg0}."),#show garrison limit for castle            
          (else_try),
            (ge, ":center_lord", 0),
            (str_store_string,s11,"@ You see the banner of {s7} over the castle gate.Current garrison limit:{reg0}."),
          (else_try),
        ...
        (else_try),
          ...
          (try_begin),         
            (eq, ":center_lord", "trp_player"),
            (str_store_string,s11,"@ Your own banner flies over the town gates.Current garrison limit:{reg0}."),            
          (else_try),
            (ge, ":center_lord", 0),
            (str_store_string,s11,"@ You see the banner of {s7} over the town gates.Current garrison limit:{reg0}."),           
          (else_try),
		    (is_between, ":center_faction", kingdoms_begin, kingdoms_end),
    ...
    ## CC
      ...
      ("center_build_barrack",[#(eq, reg6, 0),
       ...
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_barrack),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ###dormitory
      #("center_build_dormitory",[#(eq, reg6, 0),
                                      #(this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      #(party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      ## CC
                                      #(party_get_slot, ":improvement_level", "$g_encountered_party", slot_center_has_dormitory),
                                      #(lt, ":improvement_level", 2),
                                      #(val_add, ":improvement_level", 1),
                                      #(call_script, "script_get_improvement_details", slot_center_has_dormitory, "$g_encountered_party", ":improvement_level"),
                                      #(str_store_string, s3, s0),
                                      ## CC
                                       #],
       #"Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_barrack),
...
##        (else_try),
	 ...
	 #          (val_add, ":prosperity", 5),
        ...
        (try_begin),
          ...
          (try_begin),
            (eq, ":center_lord", "trp_player"),
            (str_store_string,s11,"@ Your own banner flies over the castle gate.Current garrison limit:{reg0}."),
		  (else_try),
			(gt, ":center_lord", -1),
...
##            (str_store_string,s11,"@ This castle seems to belong to no one."),
            (str_store_string,s11,"@ This castle has no garrison.Current garrison limit:{reg0}."),
          (try_end),
        (else_try),
        (call_script, "script_party_get_ideal_size", "$current_town"),
          (try_begin),
...
##            (str_store_string,s11,"@ The townsfolk here have declared their independence."),            
            (str_store_string,s11,"@ This town has no garrison.Current garrison limit:{reg0}."),
          (try_end),
        (try_end),
